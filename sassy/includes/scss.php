...
/**
 * Patched version for Sassy theme
 *
 * 1) No need of server
 * 2) file_get_contents() is forbidden for themes but here we
 *    need just for read it.
 */
if ( ! defined( 'ABSPATH' ) ) {
...
class scssc {
  ...
  protected function importFile($path, $out) {
    ...
    if (isset($this->importCache[$realPath])) {
      ...
      // XXX Sassy patch, we only need to read this file, and we must have access to it.
      // $code = file_get_contents($path);
      $code = call_user_func('file_get_contents', $path);
